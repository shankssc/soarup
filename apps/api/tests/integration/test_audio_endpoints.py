...
def mock_audio_pipeline(
    ...
    with (
        ...
        patch(_PUBLISH_EVENT, new=AsyncMock(side_effect=publish_side_effect)),
    ):
...
class TestHappyPath:
    ...
    async def test_transcription_complete_published_with_transcript(self):
        ...
        with mock_audio_pipeline(transcript=MOCK_TRANSCRIPT, publish_side_effect=track_publish):
            await _process_audio_update_async(task, UPDATE_ID)
